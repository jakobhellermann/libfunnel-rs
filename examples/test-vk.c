...
int main(int argc, char **argv) {
  ...
  while (!quit) {
...

    {
      ...
      vkCmdBindPipeline(element->commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                        pipeline);
      ...
      vkCmdPipelineBarrier(
          element->commandBuffer, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT,
          VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, 0, 0, NULL, 0, NULL, 1, &barrier);
    }
    ...
    if (buf) {
      ...
      if (ret < 0)
        ...
        fprintf(stderr, "Buffer dropped (stream renegotiated or paused)\n");
  ...
  GET_EXTENSION_FUNCTION(vkDestroyDebugUtilsMessengerEXT)(instance,
                                                          debugMessenger, NULL);
